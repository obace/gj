...
# Updated: 2025-04-05 (Removed IPv6, confirmed service start logic)
...
$(cat "$TEMP_IPV4_LIST" | sed 's/$/,/')
    ...
    chain input {
        ...
        # Allow ICMP (Ping, etc.) for IPv4
        ip protocol icmp accept comment "Allow IPv4 ICMP"

        # --- IMPORTANT: Allow SSH access ---
        # This rule allows SSH from ANY IP address.
        # Consider restricting this further if needed (e.g., ip saddr { your_static_ip } ...)
...
EOF
    ...
    # You might want to review these if your server acts as a router or needs specific output filtering.
    # chain forward {
...
main() {
    ...
    check_deps          # 检查依赖并确保 nftables 服务运行
    get_user_input      # 获取用户端口配置
    download_ip_lists   # 下载中国 IPv4 列表
    apply_nft_rules     # 生成并应用 nftables 规则
